...
exports.saveComment = function(req, res, next) {
    ...
    comment.save(function(err, result) {
        ...
        if (socketId) {
            var notificationObj = {
                noticeName: req.session.user.username + "在"+title+"文章中评论了您:",
                dir: "auto",
                // lang: '<a href:"/">请点击</a>',
                // tag: '<a href:"/">请点击</a>',
                icon: 'http://' + req.app.locals.config.host + ':' + req.app.locals.config.port + '/public/img/3.jpg',
                body: comment.content,
                //data: comment.content,
                url : '/showArticle?aid=' + aid
            };
            sio.io.sockets.sockets[socketId].emit('myMsg', JSON.stringify(notificationObj));
        }
